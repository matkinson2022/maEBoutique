...
// Prendreles données de type clé-valeur et les transferer au formatdes articles
    static func articleFromDictionary(_ snapshot:QuerySnapshot)->[Article]{
        ...
        for snapshot in snapshot.documents{
            ...
            allArticles.append(Article(id:articleData[kID] as? String ?? UUID().uuidString, name: articleData[kNAME] as?String ?? "unknown", imageName: articleData[kIMAGENAME] as? String ?? "unknown", category: Category(rawValue: articleData[kCATEGORY] as? String ?? "cucumber") ?? .cucumber, description: articleData[kDESCRIPTION] as? String ?? "Description is missing",price: articleData[kPRICE] as? Double ?? 0.0))
    }
    return allArticles
}
