...
//  maEBoutique
//
//  Created by Michael Atkinson on 2021-06-01.
//
...
class FUser {
    ...
    init(_ dictionary : NSDictionary){
        ...
        firstName = dictionary[kID] as? String ?? ""
        lastName = dictionary[kID] as? String ?? ""
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (authDataResult, error) in
            ...
            if error == nil {
                authDataResult!.user.sendEmailVerification {
                    (error) in print("verification email sent error is", error?.localizedDescription)
                }
            }
    ...
    class func resetPassword(email: String, completion: @escaping (_ error: Error?) -> Void){
        Auth.auth().sendPasswordReset(withEmail: email) { (error) in completion(error)
    ...
    func userDictionaryFrom(user: FUser) -> [String : Any] {
        return NSDictionary(objects: [user.id,user.email,user.firstName,user.lastName,user.fullName,user.fullAddress ?? "",user.onBoarding,user.phoneNumber], forKeys: [kID as NSCopying, kEMAIL as NSCopying, kFIRSTNAME as NSCopying, kLASTNAME as NSCopying, kFULLNAME as NSCopying, kFULLADDRESS as NSCopying, kONBOARD as NSCopying, kPHONENUMBER as NSCopying]) as! [String : Any]
    }
